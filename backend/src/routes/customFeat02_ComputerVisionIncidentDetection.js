...
// Body camera streams analyzed in real-time; AI detects falls, slips, environmental hazards; auto-triggers SOS if no response
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalSafetyLoneWorkerMonitor / cf-computer-vision-incident-detection',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_computer_vision_incident_detection`;
try {
  await pool.query(`
...
router.post('/run', authMiddleware, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Computer vision incident detection'.
Context: Body camera streams analyzed in real-time; AI detects falls, slips, environmental hazards; auto-triggers SOS if no response
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-computer-vision-incident-detection/run error:', err);
