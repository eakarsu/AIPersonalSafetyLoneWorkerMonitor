...
// Workers create buddy pairs; AI monitors dual check-ins; if one fails to check in, buddy is alerted and encouraged to check on them
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalSafetyLoneWorkerMonitor / cf-peer-safety-networks',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_peer_safety_networks`;
try {
  await pool.query(`
...
router.post('/run', authMiddleware, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Peer safety networks'.
Context: Workers create buddy pairs; AI monitors dual check-ins; if one fails to check in, buddy is alerted and encouraged to check on them
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-peer-safety-networks/run error:', err);
