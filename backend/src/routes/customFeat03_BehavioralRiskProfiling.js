...
// Combine location patterns, communication tone, schedule load; identify workers in distress (mental health, burnout); proactive intervention offers
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalSafetyLoneWorkerMonitor / cf-behavioral-risk-profiling',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_behavioral_risk_profiling`;
try {
  await pool.query(`
...
router.post('/run', authMiddleware, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Behavioral risk profiling'.
Context: Combine location patterns, communication tone, schedule load; identify workers in distress (mental health, burnout); proactive intervention offers
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-behavioral-risk-profiling/run error:', err);
