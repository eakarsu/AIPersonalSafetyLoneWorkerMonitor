...
// Real-time multi-modal monitoring (GPS, check-in patterns, biometrics, environmental sensors); AI predicts incidents before they happen; auto-dispatches help with context-aware info
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalSafetyLoneWorkerMonitor / cf-agentic-safety-orchestration',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_agentic_safety_orchestration`;
try {
  await pool.query(`
...
router.post('/run', authMiddleware, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Agentic safety orchestration'.
Context: Real-time multi-modal monitoring (GPS, check-in patterns, biometrics, environmental sensors); AI predicts incidents before they happen; auto-dispatches help with context-aware info
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-agentic-safety-orchestration/run error:', err);
