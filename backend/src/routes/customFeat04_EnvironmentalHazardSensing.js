...
// Integration with IoT sensors (gas, temperature, radiation); real-time alerts with evacuation routing
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalSafetyLoneWorkerMonitor / cf-environmental-hazard-sensing',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_environmental_hazard_sensing`;
try {
  await pool.query(`
...
router.post('/run', authMiddleware, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Environmental hazard sensing'.
Context: Integration with IoT sensors (gas, temperature, radiation); real-time alerts with evacuation routing
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-environmental-hazard-sensing/run error:', err);
