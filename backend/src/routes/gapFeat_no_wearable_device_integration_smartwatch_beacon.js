...
// Gap Feature: No wearable device integration (smartwatch, beacon)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalSafetyLoneWorkerMonitor / gap-no-wearable-device-integration-smartwatch-beacon',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No wearable device integration (smartwatch, beacon)'.\nProject: AIPersonalSafetyLoneWorkerMonitor.\nContext: No wearable device integration (smartwatch, beacon)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-wearable-device-integration-smartwatch-beacon', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-wearable-device-integration-smartwatch-beacon', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
