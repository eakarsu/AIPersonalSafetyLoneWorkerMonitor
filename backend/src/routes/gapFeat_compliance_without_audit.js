...
// Gap Feature: Compliance without \'/audit
// Compliance without /audit-readiness-score (predict compliance audit results)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalSafetyLoneWorkerMonitor / gap-compliance-without-audit',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Compliance without \'/audit'.\nProject: AIPersonalSafetyLoneWorkerMonitor.\nContext: Compliance without /audit-readiness-score (predict compliance audit results)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['compliance-without-audit', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'compliance-without-audit', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
