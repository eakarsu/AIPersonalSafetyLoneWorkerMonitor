...
// Gap Feature: No notifications module dedicated route (relies on SOS/emergency only)
// No notifications module dedicated route (relies on SOS/emergency only)
// Project: AIPersonalSafetyLoneWorkerMonitor
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalSafetyLoneWorkerMonitor / gap-no-notifications-module-dedicated-route-relies-on-',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No notifications module dedicated route (relies on SOS/emergency only)'.\nProject: AIPersonalSafetyLoneWorkerMonitor.\nContext: No notifications module dedicated route (relies on SOS/emergency only)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-notifications-module-dedicated-route-relies-on-', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-notifications-module-dedicated-route-relies-on-', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['no-notifications-module-dedicated-route-relies-on-']
    );
