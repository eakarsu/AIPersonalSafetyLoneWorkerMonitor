...
// Gap Feature: Shifts without \'/burnout
// Shifts without /burnout-predict (identify overworked workers)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalSafetyLoneWorkerMonitor / gap-shifts-without-burnout',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Shifts without \'/burnout'.\nProject: AIPersonalSafetyLoneWorkerMonitor.\nContext: Shifts without /burnout-predict (identify overworked workers)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['shifts-without-burnout', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'shifts-without-burnout', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
