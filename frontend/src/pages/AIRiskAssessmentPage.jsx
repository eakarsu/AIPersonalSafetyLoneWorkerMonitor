...
export default function AIRiskAssessmentPage() {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to run risk assessment. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '2rem 1rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem', marginBottom: '1.5rem' }}>
        ...
        <div>
          <h1 style={{ margin: 0, fontSize: '1.75rem', fontWeight: 700, color: '#1e293b' }}>AI Risk Assessment</h1>
          <p style={{ margin: '0.25rem 0 0', color: '#64748b', fontSize: '1rem' }}>
            Analyze worker risk levels based on check-in history, incidents, and location data
          </p>
        </div>
      </div>
      ...
      <div style={{
        ...
        <strong>What this analyzes:</strong> Worker check-in frequency, missed check-ins, incident history,
        location risk factors, and time-of-day patterns to produce a comprehensive risk profile for each worker.
      </div>
