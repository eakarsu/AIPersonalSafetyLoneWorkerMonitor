...
// Gap (AI): Shifts without '/burnout
// Shifts without /burnout-predict (identify overworked workers)
import React, { useState } from 'react';

export default function GapShiftsWithoutBurnoutPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-shifts-without-burnout/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Shifts without '/burnout</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Shifts without /burnout-predict (identify overworked workers)</p>
