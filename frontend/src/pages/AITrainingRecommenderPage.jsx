...
export default function AITrainingRecommenderPage() {
  ...
  const handleRun = async () => {
    ...
    setResponse(null);
    try {
      const res = await api.post('/api/ai/training-recommender');
      if (res.data.success) {
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '2rem 1rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem', marginBottom: '1.5rem' }}>
        ...
        <div>
          <h1 style={{ margin: 0, fontSize: '1.75rem', fontWeight: 700, color: '#1e293b' }}>AI Training Recommender</h1>
          <p style={{ margin: '0.25rem 0 0', color: '#64748b', fontSize: '1rem' }}>
            Get personalized training recommendations for each worker based on their profile
          </p>
        </div>
      </div>
      ...
      <div style={{
        ...
        <strong>What this analyzes:</strong> Worker roles, incident involvement history, current certifications,
        skill gaps, compliance requirements, and peer performance benchmarks to suggest targeted training programs.
      </div>
