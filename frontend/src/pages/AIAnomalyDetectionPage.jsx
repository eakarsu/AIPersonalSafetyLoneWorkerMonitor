...
export default function AIAnomalyDetectionPage() {
  ...
  const handleRun = async () => {
    ...
    setResponse(null);
    try {
      const res = await api.post('/api/ai/anomaly-detection');
      if (res.data.success) {
      ...
      setError(err.response?.data?.message || 'Failed to detect anomalies. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '2rem 1rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem', marginBottom: '1.5rem' }}>
        ...
        <div>
          <h1 style={{ margin: 0, fontSize: '1.75rem', fontWeight: 700, color: '#1e293b' }}>AI Anomaly Detection</h1>
          <p style={{ margin: '0.25rem 0 0', color: '#64748b', fontSize: '1rem' }}>
            Detect unusual patterns in worker behavior, check-ins, and safety metrics
          </p>
        </div>
      </div>
      ...
      <div style={{
        ...
        <strong>What this analyzes:</strong> Check-in timing deviations, unusual location patterns,
        abnormal communication gaps, sudden behavioral changes, and statistical outliers in safety metrics.
      </div>
