...
export default function AIIncidentAnalysisPage() {
  ...
  const handleRun = async () => {
    ...
    setResponse(null);
    try {
      const res = await api.post('/api/ai/incident-analysis');
      if (res.data.success) {
      ...
      setError(err.response?.data?.message || 'Failed to analyze incidents. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '2rem 1rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem', marginBottom: '1.5rem' }}>
        ...
        <div>
          <h1 style={{ margin: 0, fontSize: '1.75rem', fontWeight: 700, color: '#1e293b' }}>AI Incident Analysis</h1>
          <p style={{ margin: '0.25rem 0 0', color: '#64748b', fontSize: '1rem' }}>
            Analyze incident patterns, identify root causes, and get actionable recommendations
          </p>
        </div>
      </div>
      ...
      <div style={{
        ...
        <strong>What this analyzes:</strong> All recorded incidents including type, severity, frequency,
        time patterns, and affected locations to identify root causes and provide actionable prevention strategies.
      </div>
