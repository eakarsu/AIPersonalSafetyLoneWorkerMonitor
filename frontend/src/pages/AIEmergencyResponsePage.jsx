...
export default function AIEmergencyResponsePage() {
  ...
  const handleRun = async () => {
    ...
    setResponse(null);
    try {
      const res = await api.post('/api/ai/emergency-response');
      if (res.data.success) {
      ...
      setError(err.response?.data?.message || 'Failed to generate response plan. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '2rem 1rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem', marginBottom: '1.5rem' }}>
        ...
        <div>
          <h1 style={{ margin: 0, fontSize: '1.75rem', fontWeight: 700, color: '#1e293b' }}>AI Emergency Response</h1>
          <p style={{ margin: '0.25rem 0 0', color: '#64748b', fontSize: '1rem' }}>
            Get AI-powered optimal response plans for active emergencies
          </p>
        </div>
      </div>
      ...
      <div style={{
        ...
        <strong>What this analyzes:</strong> Active alerts, worker locations, emergency contact availability,
        nearest responder proximity, and historical emergency outcomes to generate optimal response plans.
      </div>
